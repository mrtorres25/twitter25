...
def open_session(request):
    message="Bienvenido, "
    if(request.user.is_authenticated()):
        message = message+"este es su panel de inicio \"" + request.user.username + "\""
    else:
        message = message+"ha iniciado sesión correctamente como \"" + request.user.username + "\""
    # print(get_woeid("España"))
    errorcode = "success"
    context = {
        'message': message,
        'errorcode': errorcode,
...
def get_tt(request):
    ...
    api = API(auth)
    woeid=request.GET['q']
    # print(requests.get("http://woeid.rosselliot.co.nz/lookup/spain"))
    response=api.trends_place(id=woeid)
    # print(response[0]['trends'])
    # result="{ \"listt\":["
    result="["
    for i in range(10):
        print(response[0]['trends'][i]['name'])
        result=result+"{\"name\" : \""+str(response[0]['trends'][i]['name'])+"\",\"tweet_volume\": \""+str(response[0]['trends'][i]['tweet_volume'])+"\"},"

    result = result + "{\"name\":\"" + str(response[0]['trends'][9]['name']) + "\",\"tweet_volume\": \"" +str(response[0]['trends'][9]['tweet_volume']) + "\"}"
    # result=result+"]}"
    result=result+"]"
    print(result)
    return HttpResponse(result,content_type = "application/json")
    # return HttpResponse(response[0]['trends'])